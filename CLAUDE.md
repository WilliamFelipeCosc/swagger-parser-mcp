...
- `pyproject.toml` — the package definition: 9 direct runtime deps (fastmcp, requests, jsonref, python-dotenv, azure-devops, msrest, platformdirs, pydantic, mcp) and the `swagger-parser-mcp` console script. `pydantic`/`mcp` are transitive deps of `fastmcp` too, but are pinned explicitly since `internal/azure_devops/models.py` and the Tools layer import them directly (`BaseModel`, `ToolAnnotations`), matching this list's own "packages imported directly" rule. `requirements.txt` is a one-line `-e .` pointer, not a dependency list (it used to be a ~100-package `pip freeze` that included `uvloop`, which has no Windows wheels and broke installs there)
...
- `internal/azure_devops/` — Azure DevOps integration using the `azure-devops` Python SDK, split by concern:
  - `shared.py` — `_get_connection`/`_get_project`, used by both submodules below; raise `fastmcp.exceptions.ToolError` (not a bare `ValueError`) when `AZURE_DEVOPS_*` env vars are missing, so the message is always shown to the client verbatim regardless of server-wide error masking
  - `tasks.py` — Tasks/PBIs/Features/Epics (board) integration; exposes `get_tasks` (accepts an optional `parent_id` filter), `get_pbis` (also `parent_id`, filtering by parent Feature), `get_features` (also `parent_id`, filtering by parent Epic), `get_epics` (no `parent_id` — top of the hierarchy). `_get_work_items_by_type` wraps every Azure SDK call in a `try/except AzureDevOpsServiceError`, re-raised as `ToolError` (e.g. an expired PAT surfaces as a clear message instead of a raw SDK traceback)
  - `models.py` — Pydantic `WorkItem`/`Comment` models mirroring `_extract_work_item_fields`'s output shape (including the self-referential `children: list[WorkItem]`), used by the Tools layer below for structured output/validation — not by `tasks.py` itself, which still deals in plain dicts internally
...
- `azure_work_items.py` — `get_azure_devops_tasks` (filters: `id`, `parent_id`, `assignee`, `team`, `current_sprint`, `sprint`, `state`, `top`), `get_azure_devops_pbis`/`get_azure_devops_features` (same, `parent_id` scopes to the parent Feature/Epic respectively), `get_azure_devops_epics` (same minus `parent_id`, top of the hierarchy). When `id` is set, the returned item includes a `comments` list, and for PBI/Feature/Epic also a `children` list (see Comments and children below). Return type is `list[WorkItem]` (`internal/azure_devops/models.py`) so MCP clients get a real structured-output schema instead of an untyped `list`; all four are decorated with `annotations=ToolAnnotations(readOnlyHint=True, idempotentHint=True, openWorldHint=True)`, since they're pure reads against an external system
- `wiki_cache_sync.py` — `sync_azure_devops_wiki_cache(wiki_id, fetch_content=True)`, the only mutating operation (rewrites the SQLite cache). Annotated `readOnlyHint=False, destructiveHint=False, idempotentHint=True, openWorldHint=True` — not destructive since it's a full, known-safe replace of that wiki's cached rows, not data loss
