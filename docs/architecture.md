...
| `tasks.py` | `get_tasks(item_id?, parent_id?, sprint?, current_sprint?, team?, assignee?, state?, top?)`, `get_pbis(...)`/`get_features(...)` (same, `parent_id` filters by parent Feature/Epic respectively), `get_epics(...)` (same minus `parent_id`) — all built on a shared `_get_work_items_by_type` WIQL query helper; single-item lookups by `item_id` also attach a `children` list for PBI/Feature/Epic via `_get_child_work_items`. Wraps Azure SDK calls in `try/except AzureDevOpsServiceError`, re-raised as `fastmcp.exceptions.ToolError` |
| `models.py` | Pydantic `WorkItem`/`Comment` — the structured-output shape returned by the Tools layer (`services/mcp/tools/azure_work_items.py`), not used internally by `tasks.py` |
